...
if [[ $PACKER_BUILDER_TYPE =~ vmware ]]; then
    ...
    if grep -q -i "release 5" /etc/redhat-release ; then
        echo "export PATH=$PATH:/usr/sbin:/sbin" >> $VAGRANT_HOME/.bashrc
    fi
...
if [[ $PACKER_BUILDER_TYPE =~ virtualbox ]]; then
    ...
    rm -f $VAGRANT_HOME/.vbox_version
...
if [[ $PACKER_BUILDER_TYPE =~ parallels ]]; then
    ...
    mount -o loop $VAGRANT_HOME/prl-tools-lin.iso /mnt
    sh /mnt/install --install-unattended-with-deps
    umount /mnt
    rm -rf $VAGRANT_HOME/prl-tools-lin.iso
    rm -f $VAGRANT_HOME/.prlctl_version
fi
